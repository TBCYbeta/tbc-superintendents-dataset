...
%% FRONT MATTER
\title{The Broad Center Superintendent Research Dataset \\  V2: May 14, 2026}
\date{}
\begin{document}
...
\noindent Stemper, Sam and The Broad Center. Superintendent Research Dataset (v2, 2026-05-14), 2026. 
...
Because the same superintendent may have served in multiple states, our within-state identifiers alone cannot link records across state lines. To address this, we developed a disambiguation tool (in the \texttt{disambiguation/} folder) that checks whether two records sharing the same name---but in different states---refer to the same person. The tool asks a large language model to search for corroborating evidence (news articles, school board minutes, district websites, archived pages), then passes its high-confidence ``same'' predictions to a second ``critic'' model that scores each proposed match on ten criteria and filters out roughly 13\% of them as suspect. The critic can only remove predictions, never add new ones, so it strictly improves precision at the cost of some coverage. Out of 1,724 candidate cross-state name pairs examined, 264 were confirmed as the same person.

Once confirmed matches are identified, the script \textbf{\texttt{scripts/04\_assign\_global\_ids.py}} merges the within-state identifiers into a single \texttt{global\_id} for each superintendent using a union-find algorithm. We evaluated accuracy on a hand-labeled test set of 200 cases, achieving 99.0\% overall accuracy. Because the critic can only filter---never add---the test-set bounds are at least 97.4\% precision and at most 100\% coverage.
